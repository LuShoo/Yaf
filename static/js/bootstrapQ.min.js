...
bootstrapQ.modalstr=function(a){var b='<div class="modal pop_xww '+a.fade+'" id="bsmodal" tabindex="-1" role="dialog" aria-labelledby="bsmodaltitle" aria-hidden="true" style="position:fixed;top:20px;'+a.style+'">';b+=a.big?'<div class="modal-dialog modal-lg" style="'+a.mstyle+'"><div class="modal-content">':'<div class="modal-dialog" style="'+a.mstyle+'"><div class="modal-content">';var c="";if(a.head){c+='<div class="modal-header hide">';if(a.close)c+='<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>';
c+='<h4 class="modal-title" id="bsmodaltitle">'+a.title+"</h4></div>"}var d='<div class="modal-body"><p class="lead_pop text-center">'+a.msg+"</p></div>",e="";if(a.foot){e+='';if(a.btn)e+='<button type="button" class="btn btn-default bscancel">'+a.qubtn+"</button>";e+="</div>"}return b+c+d+e+"</div></div></div>"};

bootstrapQ.alert=function(a,b){
	var c=$.extend({},bootstrapQ.modaloptions);
	c.title="\u63d0\u793a";
	if(typeof a=="string")c.msg=a;
	else $.extend(c,a);$("body").append(bootstrapQ.modalstr(c));
	var d=$("#bsmodal");d.modal(c);
	qiao.on("button.bsok","click",function(){b&&b();d.modal("hide")});
	qiao.on("#bsmodal","hidden.bs.modal",function(){d.remove()});d.modal("show")};
...
bootstrapQ.modalstr=function(a){var b='<div class="modal '+a.fade+'" id="bsmodal" tabindex="-1" role="dialog" aria-labelledby="bsmodaltitle" aria-hidden="true" style="position:fixed;top:20px;'+a.style+'">';b+=a.big?'<div class="modal-dialog modal-lg" style="'+a.mstyle+'"><div class="modal-content">':'<div class="modal-dialog" style="'+a.mstyle+'"><div class="modal-content">';var c="";if(a.head){c+='<div class="modal-header" style="background-image: linear-gradient(to bottom, #f5f5f5 0%, #e8e8e8 100%);border-radius: 6px 6px 0 0;padding: 10px 15px;color: #4fb1f7;">';if(a.close)c+='<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>';
c+='<h3 class="modal-title" id="bsmodaltitle" style="color:#4fb1f7;font-size:18px">'+a.title+"</h3></div>"}var d='<div class="modal-body" style="position: relative;padding: 15px;"><p class="lead text-center" style="font-size:21px">'+a.msg+"</p></div>",e="";if(a.foot){e+='';if(a.btn)e+='<button type="button" class="btn btn-default bscancel">'+a.qubtn+"</button>";e+="</div>"}return b+c+d+e+"</div></div></div>"};*/