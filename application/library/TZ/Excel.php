...
class TZ_Excel {
    ...
    public function findAll() {
        ...
        for ($r = 1; $r <= intval($row_max); $r++) {
            ...
            while ($j < $k) {
                ...
                $cell=$this->find($index,$r);
                if($cell instanceof PHPExcel_RichText)     //富文本转换字符串,紫阳修改
    ...
    public function dump($data = array(), $excelType = '2003') {
        ...
        foreach ($data as $val) {
            ...
            foreach ($val as $v) {
                ...
                $this->_phpexcel->getActiveSheet()->setCellValueExplicit($molumn . $this->_row, $v,PHPExcel_Cell_DataType::TYPE_STRING);
                $molumn ++;
            }
            $this->_row ++;
        }
    ...
    public function dump2($data = array(), $excelType = '2003') {
        //设置附属属性，可以省略
        $this->_phpexcel->getProperties()->setCreator("heimi");
        $this->_phpexcel->getProperties()->setLastModifiedBy("heimi");
        $this->_phpexcel->getProperties()->setTitle("Office 2003 XLS Test Document");
        ...
        foreach ($data as $val) {
            ...
            foreach ($val as $v) {
                //$this->_phpexcel->getActiveSheet()->setCellValue($molumn . $this->_row, $v);
                $this->_phpexcel->getActiveSheet()->setCellValueExplicit($molumn . $this->_row, $v,PHPExcel_Cell_DataType::TYPE_STRING);
                $this->_phpexcel->getActiveSheet()->getStyle($molumn . $this->_row)->getNumberFormat()->setFormatCode("@");
                $molumn ++;
            }
            $this->_row ++;
        }
