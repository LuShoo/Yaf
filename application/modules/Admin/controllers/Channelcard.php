 ...
 *  @final 2015-10-12
 */
class ChannelcardController extends Yaf_Controller_Abstract
{
	...
	public function indexAction()
	{
		...
		$this->_view->display('channel_card_list.tpl');
	}
	...
	public function getListAction()
	{
		...
		$channel_cardHtml = $this->_view->render('channel_card_list_row.tpl');
   ...
   //添加产品页面
	...
	public function importDataFromExcelAction()
	{
		...
		foreach ($data as $key => $val)
		{
			...
			//检查是否存在在原来的卡列表里
			$card=TZ_loader::model('Card','Card')->select(['iccid:eq'=>$rowdata['iccid']],'id,type_id,expire_time','ROW');
			if(!$card){
			...
			$rowdata["card_type"]=$card['type_id'];
			$rowdata["expire_time"]=$card['expire_time'];
			...
			$update=[];
			$update['status']=2;
			TZ_loader::model('Card', 'Card')->update($update,['id:eq'=>$card['id']]);
