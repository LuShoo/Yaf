...
{
  //经销商列表
	public $cols = array('id','partnerid','app_key','master_secret','status','created_at','updated_at');
  ...
  public function getAppListAction()
  {
    ...
    $service = TZ_Loader::service('App','Push');
  ...
  public function editAppAction()
  {
    ...
    $appInfo = TZ_Loader::model('App','Push')->select(array('id:eq'=>$id),'*','ROW');
    $this->_view->assign('appInfo',$appInfo);
    $this->_view->display('app_add.tpl');
  }
  ...
  public function createAction()
  {
    ...
    //修改
    if(!empty($cols['id'])){
      $res = TZ_Loader::service('App', 'Push')->updateAppManage($cols,$cols['id']);
      die(json_encode(array('detail'=>"ok")));
    } else{  //添加
      $cols['created_at'] = date('Y-m-d H:i:s');
      TZ_Loader::service('App', 'Push')->appManage($cols);
      die(json_encode(array('detail'=>"ok")));
    }
  }
  //删除经销商
