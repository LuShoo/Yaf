...
class DeviceService{
  ...
  public function getList($conditions,$start, $size)
  {
    ...
    return TZ_Loader::model('Device', 'Device')->select($conditions);
  ...
  public function getTotal($conditions)
  {
    ...
    $countInfo = TZ_Loader::model('Device', 'Device')->select($conditions, $fields, 'ROW');
    if(!$countInfo['total']){
      return '0';
    }
  ...
  {
    return TZ_Loader::model('Device','Device')->insert($cols);
  }


  public function setMsgManage($cols,$id)
  {
    return TZ_Loader::model('Device','Device')->update($cols,array("id:eq"=>$id));
  }
}
?>